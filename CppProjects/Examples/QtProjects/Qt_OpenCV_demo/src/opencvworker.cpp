...
//    cv::cvtColor(_frameOriginal, _frameProcessed, cv::COLOR_BGR2GRAY);
    ...
    cv::split(_frameOriginal,bgr);//split source
//    cv::Mat Channel;
    bgr[ChannelNumber].copyTo(_frameProcessed);
...
//                  QImage::Format_Indexed8);
    ...
    QImage output;
    output = ASM::cvMatToQImage(_frameProcessed);
...
void OpenCvWorker::receiveChannel(int val)
{
    ChannelNumber=val;
}
