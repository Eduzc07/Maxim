        ...
        for(pos=lines->begin();pos!=lines->end();pos++) {
            ...
            if(fabs((*pos)[0]-(*current)[0])<20 && fabs((*pos)[1]-(*current)[1])<CV_PI*10/180) {
                ...
                if((*pos)[1]>CV_PI*45/180 && (*pos)[1]<CV_PI*135/180) {
                    ...
                    pt1.y=0;
                    pt1.x=p/cos(theta);
                    pt2.y=img.size().height; pt2.x=-pt2.y/tan(theta) + p/cos(theta);
                }
...
{
    Mat sudoku;
    cv::cvtColor(image, sudoku, COLOR_BGR2GRAY);
    ...
    //Print Points
    for (int i = 0; i < 4; i++)
        line(image, src[i], src[(i+1)%4], Scalar(0,255,0), 2);

    Mat undistorted = Mat(Size(maxLength, maxLength), CV_8UC1);
    Mat sudoku_gaus;
    GaussianBlur(sudoku_original, sudoku_gaus, Size(9,9), 0);
    cv::warpPerspective(sudoku_gaus, undistorted,cv::getPerspectiveTransform(src, dst), Size(maxLength, maxLength));
    box = undistorted.clone();

    Mat ohneGaus;
    cv::warpPerspective(sudoku_original, ohneGaus,cv::getPerspectiveTransform(src, dst), Size(maxLength, maxLength));
    Originalbox = ohneGaus.clone();
    return maxLength;
}
