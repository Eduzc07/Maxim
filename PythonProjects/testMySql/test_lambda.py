...
def getDate(date):
    """
    This function changes to mysql datetime format
    IMPORTANT: Always in UTC format
    """
    date, time = date.split(" ", 2)
    YY, MM, DD = date.split("-", 3)
    hh, mm, ss = time.split(":", 3)
    return [YY, MM, DD]
...
def lambda_handler(event = None, context = None):
    ...
    # print(event.values())
    name = 'Shovel01'
    [YY, MM, DD] = getDate(event["datetime"])
    folder = 'Chinaco/data/{:0>4}/{:0>2}/{:0>2}/'.format(YY, MM, DD)
    filename = '{}_{:0>4}{:0>2}{:0>2}.csv'.format(name, YY, MM, DD)
    print(folder)
    print(filename)
