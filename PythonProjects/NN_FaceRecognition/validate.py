...
sv = np.loadtxt('data/data_images/v.data', dtype=np.float32)
sw = np.loadtxt('data/data_images/w.data', dtype=np.float32)
...
def draw_rects(img, rects, color):
    ...
    for x1, y1, x2, y2 in rects:
        cv2.rectangle(img, (x1 - val, y1 - 2*val),
                      (x2 + val, y2+val), color, 2)
...
if __name__ == '__main__':
    ...
    for i in onlyfiles:
        ...
        nameToSay = "No se ha encontrado a nadie."
        ...
        for x1, y1, x2, y2 in rects:
            ...
            #nameToSay = "\'%s\' esta en la foto" %names[person]
            print("\'%s\' esta en la foto %s." % (names[person], imgName))

            val = 20
            cv2.rectangle(vis, (x1 - val, y1 - 2*val),
                          (x2 + val, y1), (0, 150, 0), -1)
            cv2.putText(vis, "%s" % names[person], (x1 - int(val/2), y1 - 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (250, 255, 250), 2)
            #############################################################################
            cv2.imshow("Image", vis)
